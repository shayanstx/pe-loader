...
void* load_PE (char* PE_data) {
    ...
    for(int i=0; import_descriptors[i].OriginalFirstThunk != 0; ++i) {
        ...
        for(int i=0; lookup_table[i].u1.AddressOfData != 0; ++i) {
            ...
            if((lookup_addr & IMAGE_ORDINAL_FLAG) == 0) { //if first bit is not 1
                ...
                function_handle = reinterpret_cast<void*>(GetProcAddress(import_module, reinterpret_cast<LPCSTR>(lookup_addr)));
            }
            ...
            address_table[i].u1.Function = reinterpret_cast<ULONGLONG>(function_handle);
        }
    ...
    uintptr_t delta_VA_reloc = reinterpret_cast<uintptr_t>(ImageBase) - p_NT_HDR->OptionalHeader.ImageBase;
    ...
    if(data_directory[IMAGE_DIRECTORY_ENTRY_BASERELOC].VirtualAddress != 0 && delta_VA_reloc != 0) {
        ...
        while(p_reloc->VirtualAddress != 0) {
            ...
            p_reloc = reinterpret_cast<IMAGE_BASE_RELOCATION*>((reinterpret_cast<IMAGE_BASE_RELOCATION*>(p_reloc)) + p_reloc->SizeOfBlock);
